...
export default function NextUpScreen() {
  ...
  const nextUpEvents = useMemo(() => {
    const now = new Date();
    const staffed = data.all.filter(event => hasMinimumPublishedStaff(event));
    const dated = staffed
      .filter((event): event is ScheduleEvent & { dateTime: Date } => Boolean(event.dateTime))
      .sort((a, b) => a.dateTime.getTime() - b.dateTime.getTime());

    const isSameDay = (a: Date, b: Date) =>
      a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate();

    const todayEvents = dated.filter(event => isSameDay(event.dateTime, now));
    if (todayEvents.length > 0) return todayEvents;

    const upcoming = dated.filter(event => event.dateTime.getTime() >= now.getTime());
    if (upcoming.length > 0) {
      const targetDate = upcoming[0].dateTime;
      return upcoming.filter(event => isSameDay(event.dateTime, targetDate));
    }

    if (dated.length > 0) {
      const targetDate = dated[0].dateTime;
      return dated.filter(event => isSameDay(event.dateTime, targetDate));
    }

    return staffed.slice(0, 1);
  }, [data.all]);
...
function EventCard({
  ...
  return (
    <Pressable
      ...
      <Text style={[styles.teamText, { color: teamMeta.themeColor }]}>{teamMeta.title}</Text>
      <View style={styles.themeRow}>
        <TeamThemeIcon team={event.team} color={teamMeta.themeColor} />
...
function TeamThemeIcon({ team, color }: { team: keyof typeof TEAM_META; color: string }) {
  if (team === 'cherry_bombs') {
    return <MaterialCommunityIcons name="bomb" size={20} color={color} />;
  }
  if (team === 'pickles') {
    return <MaterialIcons name="sports-baseball" size={20} color={color} />;
  }
  return <MaterialCommunityIcons name="food-hot-dog" size={20} color={color} />;
}
